...
export function SolidButton({icon, children, ...rest} : ButtonModelProps){
    ...
    return (
        <Button px="8" w="fit-content" borderRadius="1px" fontWeight="semibold" leftIcon={icon && <Icon as={icon} stroke="#ffffff" fontSize="16" fill="none" mr="2"/>} variant="solid" fontSize="sm" {...rest} _hover={{boxShadow: `0 8px 20px -8px ${boxShadowColor}`}} _active={{}}>
            {children}
        </Button>
    )
}