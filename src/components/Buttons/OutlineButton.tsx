...
export function OutlineButton({icon, children, hasAnimation = false, ...rest} : ButtonModelProps){
    return (
        <Button px="10" w="fit-content" pos="relative" leftIcon={icon && <Icon as={icon} stroke="#ffffff" fontSize="lg" fill="none" mr="2"/>} fontSize="sm" variant="outline" fontWeight={"normal"} borderRadius="1px" borderColor="gray.800" border="1px" 
        _hover={{borderColor: "gray.600", bg: "rgba(255,255,255,0.04)", color: "#fff", _before: {w: "100%"}, _after: {w: "100%"}, transition: "all ease 0.7s" }} transition="all ease 0.7s"
